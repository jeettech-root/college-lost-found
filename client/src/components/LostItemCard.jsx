...
import { MapPinIcon, CalendarIcon, DollarSignIcon, TagIcon, EditIcon, TrashIcon, ImageIcon } from './Icons';
...
export default function LostItemCard({ item, currentUserId, onDelete }) {
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-3xl border border-white/[0.08] bg-[#111726]/90 shadow-glow backdrop-blur-sm transition duration-300 hover:-translate-y-1 hover:border-amber-500/30 hover:shadow-amber-subtle">
      ...
      <div className="relative aspect-[16/9] w-full overflow-hidden bg-slate-900">
        {item.image ? (
          ...
          <div className="flex h-full w-full flex-col items-center justify-center bg-gradient-to-br from-amber-950/40 via-[#111726] to-slate-900 p-6 text-center">
            ...
            <p className="mt-2 text-[11px] font-bold uppercase tracking-widest text-amber-400/80">Lost Item Report</p>
          </div>
        )}

        {/* Top Badges */}
        <div className="absolute left-3 top-3 flex items-center gap-2">
          <span className="rounded-xl border border-amber-500/30 bg-[#0B0F17]/90 px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider text-amber-300 backdrop-blur-md">
            Lost
          </span>
        </div>
      ...
      <div className="flex flex-1 flex-col justify-between p-5 space-y-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            {item.reward ? (
              <span className="flex items-center gap-1 rounded-lg border border-amber-500/30 bg-amber-500/10 px-2 py-0.5 text-[11px] font-bold text-amber-300">
          ...
          <h3 className="mt-2 text-lg font-bold text-white transition group-hover:text-amber-300 line-clamp-1">
            {item.title}
          </h3>
        ...
        <div className="grid grid-cols-2 gap-2 text-xs text-slate-300">
          <div className="flex items-center gap-2 rounded-xl border border-white/[0.06] bg-white/[0.03] px-3 py-2">
            <MapPinIcon className="h-4 w-4 shrink-0 text-amber-400" />
        ...
        <div className="pt-2">
          <div className="flex items-center gap-2">
            <Link
              to={`/lost/${item._id}`}
              className="flex-1 rounded-xl bg-amber-400 px-4 py-2.5 text-center text-xs font-bold text-slate-950 transition hover:bg-amber-300 shadow-amber-subtle"
            >
              View Details
            </Link>
